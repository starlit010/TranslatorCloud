...
function creatmark_list(){
	$("#mark_list").remove();
    var markMenu="<div id='mark_list' style='display:none;z-index:999999;background:#fff;color:#999;box-shadow: 1px 1px 10px 0 #b8b8b9;position:fixed;top:45px;right:250px;width:230px;max-height:250px;padding:2px 0'>"+
...
function postMessageListener(e){
	...
	if(e.data.action=="YEEKIT_SUFFER_SOURCE"){
		...
		loginInfos.userName=e.data.userName;
		loginInfos.id=e.data.userId;
		show_star();
		// login_stats();
...
function list_translation(src,tgt,texts,nodes,translateId){
	...
	if(translateId != undefined){
		...
		console.log({action:"list_translate_action",src:src,tgt:tgt,texts:texts,translateId:translateId,website:window.location.hostname,weburl:window.location.href});
